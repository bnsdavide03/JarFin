...
\begin{lstlisting}[language=Java, caption={Core dell'algoritmo di aggregazione}, label={lst:algo_core}]
...
\begin{lstlisting}[language=Java, caption={Logica di valutazione finanziaria}, label={lst:alert}]
...
Data la natura interconnessa del servizio, un semplice Unit Test non sarebbe sufficiente se richiedesse il servizio Accounting attivo. Per garantire che i test siano deterministici e veloci, è stato utilizzato Mockito.
...
Il test verifica la correttezza matematica dell'aggregatore fornendo dati fittizi ("Stubbing"):
...
public void testGenerateReport_Success() {
    ...
    when(restTemplate.getForObject(anyString(), eq(TransactionDTO[].class)))
            .thenReturn(new TransactionDTO[]{t1, t2});
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{../Iterazione_2/images/JUnit.png}
    \caption{Esito positivo dei test unitari su AnalyticsService.}
    \label{fig:junit_analytics}
\end{figure}

Questo approccio disaccoppia il test della logica di business dalla disponibilità della rete o del database, rispettando la piramide dei test.

\section{Validazione Funzionale del Servizio}

Una volta verificata la logica unitaria, è stato avviato il servizio per una verifica funzionale completa.

\subsection{Avvio del Microservizio}
Come mostrato nel log di avvio (Figura \ref{fig:run_analytics}), il servizio viene inizializzato correttamente sulla porta \texttt{8082}, come definito nel file \texttt{application.properties}.



\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{../Iterazione_2/images/run_analytics_service.png}
    \caption{Console log: avvio di AnalyticsService sulla porta 8082.}
    \label{fig:run_analytics}
\end{figure}

\subsection{Verifica Output JSON}
Utilizzando Postman, è stata simulata una richiesta di report. \\
La risposta (Figura \ref{fig:postman_report}) dimostra che:
\begin{enumerate}
    \item I dati vengono recuperati correttamente dall'Accounting Service.
    \item L'algoritmo di aggregazione calcola i totali (es. \texttt{totalBalance}, \texttt{totalExpenses}).
    \item Il sistema di alert funziona, restituendo un livello \texttt{GREEN} (poiché le entrate superano le spese superando il margine minimo impostato del 20\%).
\end{enumerate}
...
\begin{figure}[H]
    ...
    \caption{Risposta JSON completa del report finanziario.}
    \label{fig:postman_report}
\end{figure}

\section{Conclusione Iterazione 2}
L'Iterazione 2 ha prodotto un servizio di Analytics capace di elaborare dati complessi con efficienza lineare $O(n)$. L'uso di configurazioni externalizzate e pattern di test isolati garantisce che il servizio sia pronto per il deployment in ambiente containerizzato \textit{JarFin}.